...
const FORTINET_RSS_URL = "https://support.fortinet.com/rss/Software.xml";
...
const ui = initSoftwareLifecycleUI({
...
initToolboxNav({ current: "software-lifecycle", basePath: "../" });
initThemeToggle();
bootstrap();

async function bootstrap() {
  try {
    const persisted = await loadSoftwareLifecyclePersisted();
    if (persisted) {
      ({ rows } = persisted);
      rowsById = new Map(rows.map((row) => [row.id, row]));
      miniSearch = loadLifecycleSearchIndex(persisted.indexJSON);
      meta = persisted.meta;
      const storedBytes = meta?.storedBytes ?? estimateSoftwareLifecycleSizeBytes(rows, persisted.indexJSON);
    ...
    console.error("Failed to restore Software LifeCycle data", error);
    ui.renderDatasetEmpty();
    ui.showStatus("error", error.message || "Failed to load stored Software LifeCycle data.");
  }
}
...
async function handleImportText(xmlText) {
  ...
  try {
    const result = parseSoftwareLifecycleRssText(source);
    await applyDataset(result, {
      sourceLabel: "Pasted Fortinet Software RSS XML"
    });
    ui.clearPasteInput();
    ui.closeModal();
    ui.showStatus(
    ...
    console.error("Failed to import pasted Software LifeCycle XML", error);
...
async function handleImportClipboard() {
  ...
  try {
    ...
    try {
      const result = parseSoftwareLifecycleRssText(clipboardText);
      await applyDataset(result, {
        sourceLabel: "Clipboard Fortinet Software RSS XML"
      });
      ui.clearPasteInput();
      ui.closeModal();
      ui.showStatus(
      ...
      console.error("Failed to import clipboard Software LifeCycle XML", error);
      ui.setPasteInputValue(clipboardText);
      ui.showModalStatus(
        "warn",
        "Clipboard content did not parse as Fortinet software RSS XML. Review or replace it below, then press Ctrl+Enter to import.",
        { dismissAfter: 7000 }
      );
    } finally {
      ui.setLoading(false);
...
async function handleClear() {
  if (!rows.length && !miniSearch) {
    ui.showStatus("info", "Software LifeCycle data is already empty.");
    return;
  }
  ...
  ui.showStatus("success", "Cleared stored Software LifeCycle data.");
}
...
async function applyDataset(result, options = {}) {
  ...
  const storedBytes = estimateSoftwareLifecycleSizeBytes(rows, exported);
  meta.storedBytes = storedBytes;
  await saveSoftwareLifecyclePersisted(rows, exported, meta);
