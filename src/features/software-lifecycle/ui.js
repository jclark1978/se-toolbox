...
export function initSoftwareLifecycleUI(handlers) {
  ...
  const openModalButton = document.getElementById("slc-open-modal-button");
  const openButton = document.getElementById("slc-open-button");
  const pasteClipboardButton = document.getElementById("slc-paste-clipboard-button");
  const closeModalButton = document.getElementById("slc-close-modal-button");
  const modal = document.getElementById("slc-modal");
  const pasteInput = document.getElementById("slc-paste-input");
  const modalStatusEl = document.getElementById("slc-modal-status");
  const clearButton = document.getElementById("slc-clear-button");
  const searchInput = document.getElementById("slc-search-input");
  const searchSummary = document.getElementById("slc-search-summary");
  const resultsBody = document.getElementById("slc-results-body");
  const spinner = document.getElementById("slc-dataset-spinner");
  const statusEl = document.getElementById("slc-dataset-status");

  const datasetFeed = document.getElementById("slc-dataset-feed");
  const datasetRows = document.getElementById("slc-dataset-rows");
  const datasetUpdated = document.getElementById("slc-dataset-updated");
  const datasetSize = document.getElementById("slc-dataset-size");
  ...
  clearButton.addEventListener("click", () => {
    ...
    const confirmed = window.confirm("Clear the stored Software LifeCycle data?");
    if (confirmed) {
      onClear();
    }
  });
  ...
  function renderResults(rows, summary) {
    ...
    if (!rows.length) {
      ...
      if (!datasetLoaded) {
        ...
        td.textContent = "Enter a software release to see milestone dates.";
      }
      ...
      for (const row of rows) {
        ...
        releaseTd.textContent = formatDateDisplay(row.releaseDate);
        tr.appendChild(releaseTd);

        const eoeTd = document.createElement("td");
        eoeTd.textContent = formatDateDisplay(row.endOfEngineeringSupportDate);
        tr.appendChild(eoeTd);
